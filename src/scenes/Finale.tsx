...
export function Finale() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      setProgress(p)
      if (p >= 1) {
        window.setTimeout(() => setPhase('credits'), 1500)
        return
      }
  ...
  return (
    <div className="finale-wrap">
      <Canvas camera={{ position: [0, 0, 10], fov: 70 }} dpr={[1, 2]}>
        ...
        <pointLight color="#c470ff" intensity={6} distance={100} position={[0, 0, -10]} />
        <pointLight color="#ffd700" intensity={4} distance={50} position={[5, 5, -10]} />
        <TimelineCollapse progress={progress} />
        <Particles count={1500} color="#c470ff" size={0.08} radius={30} shape="sphere" speed={0.3} />
...
function TimelineCollapse({ progress }: { progress: number }) {
  const group = useRef<THREE.Group>(null)
  const core = useRef<THREE.Mesh>(null)
  const erasRef = useRef<THREE.Group[]>([])
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime()
    if (group.current) group.current.rotation.y = t * 0.2
    if (core.current) {
      core.current.scale.setScalar(1 + Math.sin(t * 4) * 0.15 + progress * 2)
      const mat = core.current.material as THREE.MeshStandardMaterial
      mat.emissiveIntensity = 2 + Math.sin(t * 5) * 1 + progress * 3
    }
    erasRef.current.forEach((g, i) => {
      if (!g) return
      ...
      if (firstChild && firstChild.material) {
        ;(firstChild.material as THREE.MeshStandardMaterial).emissiveIntensity =
          0.4 + Math.sin(t * 2 + i) * 0.3 + progress * 1.2
      }
    })
  })
  return (
    <group>
      <group ref={group}>
        <mesh ref={core} position={[0, 0, -12]}>
          <icosahedronGeometry args={[1, 1]} />
          <meshStandardMaterial color="#ffd700" emissive="#ffd700" emissiveIntensity={3} metalness={0.8} roughness={0.2} />
        </mesh>
...
function SpinningRings({ progress }: { progress: number }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (ref1.current) {
      ...
      ref1.current.scale.setScalar(2 + progress * 3)
    }
    if (ref2.current) {
      ref2.current.rotation.x = -t * 0.5
      ref2.current.rotation.z = t * 0.7
      ref2.current.scale.setScalar(3 + progress * 4)
    }
  })
  return (
